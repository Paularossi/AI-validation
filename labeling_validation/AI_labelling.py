...
# DIGITAL CODING PROBLEMS:
# WHO category contains weird subletters?? 16f, 18b, 14i, etc.
# where is processing level? if it's UPF_Euro_FoodCode, then again there are weird numbers 0, 16f, 11 (only 4 in total)
...
AD_PATTERN = re.compile(r"(.+?)\.(png|jpeg|jpg)") # re.compile(r"(\d+)\.(png|jpeg|jpg)") 
...
def create_first_payload(image, all_questions, ad_creative_bodies = None):
    image_path = os.path.join(image_folder, image)
    base64_image = encode_image(image_path)
    image_url = f"data:image/jpeg;base64,{base64_image}"
    ...
    messages = [{"role": "system", "content": instructions_1 if ad_creative_bodies is None else instructions_1_text}]
    user_content = [{"type": "text", "text": q[0] + ": " + q[1]} for q in shuffled_questions] 
    user_content.append({"type": "image_url", "image_url": {"url": image_url}})
    if ad_creative_bodies is not None:
...
def process_images(images, captions = None):
    ...
    for image in images:
        ...
        if "is_ad" in dict_entry_intro and dict_entry_intro["is_ad"] == '1':
            ...
            first_payload = create_first_payload(image, all_questions, captions[captions["img_id"] == ad_id]["ad_creative_bodies"].values[0])
            try:
...
sampled_images = random.sample(images, 20)
...
labeling_outputs_intro, responses_intro, labeling_outputs_tree, labeling_outputs_two, responses_first_prompt, responses_second_prompt = process_images(sampled_images, ads_subset)
