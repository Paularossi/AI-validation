...
label_df <- read.csv("C:/Users/P70090005/OneDrive - Maastricht University/Desktop/phd/AI-validation/data/label_mappings.csv", 
...
merge_ai_human_column <- function(models, humans = NULL, column, dieticians = NULL) {
  ...
  model_preds_list <- lapply(model_names, function(model) {
    df <- as.data.table(models[[model]])
    col <- ifelse(column != "brand", ai_mapping[[column]], column)
    if (col %in% names(df)) {
      setnames(df[, .(img_id, get(col))], c("img_id", model))
    } else {
      data.table(img_id = character(0))
    }
  })

  model_preds <- Reduce(function(x, y) merge(x, y, by = "img_id", all = TRUE), model_preds_list)
  result <- model_preds
  ...
  if (!is.null(humans)) {
    ...
    if (consensus_col %in% names(humans)) {
      human_cons <- as.data.table(humans)[, .(img_id, consensus = get(consensus_col))]
      result <- merge(result, human_cons, by = "img_id", all = TRUE)
    }
  }
...
masi <- function(x, y, sep = ", ", jaccard_only = FALSE, type = "sim") {
  ...
  # monotonicity similarity coefficient, M, see http://www.lrec-conf.org/proceedings/lrec2006/pdf/636_pdf.pdf Passonneau. 2006
